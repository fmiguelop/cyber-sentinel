...
export function DefconIndicator({ level, size = "sm" }: { level: number; size?: "sm" | "xs" }) {
  ...
  return (
    <div
      className={`${colorClass} ${sizeClass} shrink-0 rounded-full ${level <= 2 ? "ring-offset-card ring-2 ring-red-500/50 ring-offset-1" : ""}`}
      aria-hidden="true"
    />
  );
}
...
function TrendArrow({ now, global }: { now: number; global: number }) {
  ...
  return (
    <span className="ml-1 text-[10px] text-gray-500" role="img" aria-label={label}>
      {direction === "up" ? "↑" : "↓"}
    </span>
  );
}
  ...
  return (
    <div role="region" aria-label="DEFCON Status">
      <button
        ...
        className={cn(
          "focus-visible:ring-ring w-full cursor-pointer rounded-sm text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2",
          isExpanded && "mb-4"
        )}
        aria-expanded={isExpanded}
        aria-label={`DEFCON ${defconLevel} - ${isExpanded ? "Collapse" : "Expand"} details`}
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <DefconIndicator level={defconLevel} size="sm" />
            <span className="text-foreground text-sm font-semibold">DEFCON {defconLevel}</span>
          </div>
          <ChevronDown
            className={cn(
              "hidden h-4 w-4 text-zinc-500 transition-transform duration-200 md:block",
              isExpanded && "rotate-180"
            )}
          />
        </div>
        <div className="text-muted-foreground mt-1 text-xs">{defconSubtitle}</div>
      </button>
      ...
      >
        ...
        <div className="mt-4 space-y-3">
          ...
          >
            <div className="text-muted-foreground mb-2 grid grid-cols-[1fr_5rem_5rem] gap-4">
              <span></span>
              <span className="text-right font-mono">Global</span>
              <span className="text-right font-mono text-yellow-500">Now</span>
            </div>

            <div className="space-y-0" role="rowgroup">
              <div className="grid grid-cols-[1fr_5rem_5rem] items-center gap-4 py-1.5" role="row">
                <span className="text-gray-300" role="cell">
                  Active Threats
                </span>
                <span className="text-right font-mono text-white tabular-nums" role="cell">
                ...
                >
                  ...
                  {!nowIsEmpty && (
                    <TrendArrow now={filteredThreats.length} global={activeThreats.length} />
                  )}
                </span>
              </div>
              ...
              <div className="grid grid-cols-[1fr_5rem_5rem] items-center gap-4 py-1.5" role="row">
                ...
                <span
                  className="truncate text-right font-mono text-white"
                  role="cell"
                ...
                <span
                  className={`truncate text-right font-mono ${nowIsEmpty ? "text-gray-600" : "text-yellow-500"}`}
                  role="cell"
              ...
              <div className="grid grid-cols-[1fr_5rem_5rem] items-center gap-4 py-1.5" role="row">
                ...
                <span className="text-right font-mono text-white" role="cell">
