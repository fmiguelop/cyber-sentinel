...
export function DefconIndicator({ level, size = "sm" }: {
    level: number;
    size?: "sm" | "xs";
}) {
    const colorClass = getDefconColor(level);
    const sizeClass = size === "sm" ? "h-3 w-3" : "h-2 w-2";
    return (<div className={`${colorClass} ${sizeClass} rounded-full shrink-0 ${level <= 2 ? "ring-2 ring-offset-1 ring-offset-card ring-red-500/50" : ""}`} aria-hidden="true"/>);
}
export { getDefconLevel, getDefconSubtitle, getDefconColor };
function DefconHeaderContent({ disableTooltip = false }: {
    ...
    const tooltipContent = (<TooltipContent side="right" className="max-w-sm">
    ...
    return (<TooltipProvider delayDuration={300}>
      <div className="border-b border-border px-6 py-3 group">
        <div className="flex items-center gap-2">
          ...
          {disableTooltip ? (<button type="button" className="focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-sm cursor-default opacity-50" aria-label="DEFCON levels info (tooltip disabled on mobile)" disabled>
              <Info className={`h-4 w-4 text-gray-400 ${showPulse ? "animate-pulse" : ""}`}/>
            </button>) : (<Tooltip onOpenChange={handleTooltipOpenChange}>
              <TooltipTrigger asChild>
                <button type="button" className="focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-sm cursor-pointer" aria-label="Learn about DEFCON levels">
                  <Info className={`h-4 w-4 text-gray-400 transition-colors hover:text-gray-200 ${showPulse ? "animate-pulse" : ""}`}/>
                </button>
              </TooltipTrigger>
              {tooltipContent}
            </Tooltip>)}
...
export function DefconCard({ disableTooltip = false }: {
    disableTooltip?: boolean;
}) {
    return (<Card className="border-border bg-card shadow-lg" role="region" aria-label="DEFCON Status">
...
function TrendArrow({ now, global }: {
    now: number;
    global: number;
}) {
    if (now === global)
    ...
    return (<span className="ml-1 text-[10px] text-gray-500" role="img" aria-label={label}>
      {direction === "up" ? "↑" : "↓"}
    </span>);
}
export function StatHUD() {
    ...
    const timeRangeLabel = filters.timeRange === "all"
        ? "All Time"
        : filters.timeRange === "1min"
            ...
            : filters.timeRange === "5min"
                ? "Last 5min"
                : "Last 1hr";
    const hasNarrowingFilters = activeSeverityBadges.length > 0 || activeAttackTypeBadges.length > 0 || filters.timeRange !== "all";
    return (<Card className="border-border bg-card shadow-lg" role="region" aria-label="Threat Statistics">
      <DefconHeaderContent />
      <CardContent className="space-y-3">
        
        <div className="pb-2 text-xs" role="table" aria-label="Threat metrics comparison table: Global versus Now">
          ...
          <div className="space-y-0" role="rowgroup">
            ...
            <div className="grid grid-cols-[1fr_5rem_5rem] gap-4 items-center py-1.5" role="row">
              ...
              <span className={`text-right font-mono tabular-nums ${nowIsEmpty && filteredThreats.length === 0
            ? "text-gray-600"
            : isSignificantDiff(filteredThreats.length, activeThreats.length)
                ? "text-yellow-300 font-semibold"
                : "text-yellow-500"}`} role="cell" aria-label={`Now ${filteredThreats.length}, global ${activeThreats.length}`}>
            ...
            <div className="grid grid-cols-[1fr_5rem_5rem] gap-4 items-center py-1.5" role="row">
              ...
              <span className={`text-right font-mono tabular-nums ${nowIsEmpty && statsFiltered.activeCritical === 0
            ? "text-gray-600"
            : isSignificantDiff(statsFiltered.activeCritical, statsGlobal.activeCritical)
                ? "text-yellow-300 font-semibold"
                : "text-yellow-500"}`} role="cell" aria-label={`Now critical ${statsFiltered.activeCritical}, global critical ${statsGlobal.activeCritical}`}>
                {nowIsEmpty && statsFiltered.activeCritical === 0 ? "--" : statsFiltered.activeCritical}
                {!nowIsEmpty && (<TrendArrow now={statsFiltered.activeCritical} global={statsGlobal.activeCritical}/>)}
              </span>
            </div>
            ...
            <div className="grid grid-cols-[1fr_5rem_5rem] gap-4 items-center py-1.5" role="row">
              ...
              <span className="text-right text-white font-mono truncate" role="cell" title={toCountryCode(statsGlobal.topSourceCountry)}>
                {toCountryCode(statsGlobal.topSourceCountry)}
              </span>
              <span className={`text-right font-mono truncate ${nowIsEmpty ? "text-gray-600" : "text-yellow-500"}`} role="cell" title={toCountryCode(statsFiltered.topSourceCountry)}>
        ...
        <div className="pt-3 space-y-2">
          ...
          {hasNarrowingFilters ? (<div className="flex flex-wrap items-center gap-2">
              {activeSeverityBadges.map((sev) => {
                const label = sev.charAt(0).toUpperCase() + sev.slice(1);
                const sevClasses = sev === "critical"
                    ? "border-red-500/40 bg-red-500/10 text-red-300"
                    : sev === "medium"
                        ? "border-yellow-500/40 bg-yellow-500/10 text-yellow-300"
                        : "border-green-500/40 bg-green-500/10 text-green-300";
                return (<Badge key={sev} variant="outline" className={`font-mono ${sevClasses}`} aria-label={`Severity filter: ${label}`}>
                    {label}
                  </Badge>);
            })}

              {activeAttackTypeBadges.map((type) => (<Badge key={type} variant="outline" className="font-mono border-gray-700 bg-gray-800/40 text-gray-200" aria-label={`Attack type filter: ${type}`}>
                  {type}
                </Badge>))}
          ...
          {nowIsEmpty && hasNarrowingFilters ? (<div className="text-xs text-gray-500">No threats match current filters</div>) : null}
        </div>
      </CardContent>
    </Card>);
}
