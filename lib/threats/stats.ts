...
export function computeThreatStats(threats: ThreatEvent[]): ThreatStats {
    ...
    const topSourceCountry = Object.keys(sourceCountryCounts).length > 0
        ? Object.entries(sourceCountryCounts).reduce((a, b) => a[1] > b[1] ? a : b)[0]
        : null;
    const topSourceRegion = Object.keys(sourceRegionCounts).length > 0 &&
        Object.values(sourceRegionCounts).some((count) => count > 0)
        ? (Object.entries(sourceRegionCounts).reduce((a, b) => a[1] > b[1] ? a : b)[0] as City["region"])
        : null;
    const now = Date.now();
    const activeCritical = threats.filter((threat) => threat.severity === "critical" && threat.timestamp + threat.duration > now).length;
    return {
        totalAttacks,
        activeCritical,
        byRegion,
