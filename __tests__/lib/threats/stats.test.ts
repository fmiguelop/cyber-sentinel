...
describe("stats utilities", () => {
    const createMockThreat = (severity: ThreatEvent["severity"], type: ThreatEvent["type"], sourceRegion: ThreatEvent["source"]["region"], sourceCountry: string): ThreatEvent => {
        const sourceCity = CITIES.find((c) => c.region === sourceRegion && c.country === sourceCountry) || CITIES[0];
