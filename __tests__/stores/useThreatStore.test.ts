...
describe("useThreatStore", () => {
  ...
  const createMockThreat = (
    severity: ThreatEvent["severity"] = "low",
    type: ThreatEvent["type"] = "DDoS"
  ): ThreatEvent => {
    return {
      id: `test-${Math.random()}`,
      timestamp: Date.now(),
  ...
  describe("setFilters", () => {
    ...
    it("should trigger updateMapFeatures when filters change", () => {
      ...
      // Fast-forward past debounce again
      act(() => {
        jest.advanceTimersByTime(150);
      });
  ...
  describe("pruneExpired", () => {
    it("should remove expired threats from activeThreats", () => {
      ...
      const activeThreat: ThreatEvent = {
        ...createMockThreat(),
        timestamp: now - 2000,
        duration: 5000, // Still active
      };
      const expiredThreat: ThreatEvent = {
        ...createMockThreat(),
        timestamp: now - 10000,
        duration: 5000, // Expired
      };
  ...
  describe("updateMapFeatures", () => {
    it("should debounce map feature updates", () => {
      ...
      // Should not have updated yet (debounce is 100ms)
